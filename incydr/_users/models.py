...
class User(ResponseModel):
    ...
    user_id: Optional[str] = Field(
        alias="userId", description="A globally unique ID for this user."
    )
    username: Optional[str] = Field(
        description="The name the user uses to log in to Code42."
    )
    first_name: Optional[str] = Field(
        alias="firstName", description="The first (given) name of the user."
    )
    last_name: Optional[str] = Field(
        alias="lastName", description="The last (family) name of the user."
    )
